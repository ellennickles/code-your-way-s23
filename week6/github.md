...
2. If you're using macOS Sierra 10.12.2 or later, you will need to modify your
   ...
   - Print out the contents of the file to verify the change:
      ...
      the terminal (another text editor) to modify the config file with the above lines:
...
4. Verify that the resulting message contains your username. If you receive a "permission denied" message, see "[Error: Permission denied (publickey)](https://docs.github.com/en/authentication/troubleshooting-ssh/error-permission-denied-publickey)."
...
If you want to pull a local version of your repository to different place, e.g
another local computer or a server on a hosting service, or if you want a local
